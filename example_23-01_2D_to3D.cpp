...
static void StereoCalib(const char *imageList, int nx, int ny,
  ...
  if (showUndistorted) {
    ...
    //
    if (!useUncalibrated) {
      std::cout << "inside !useUncalibrated\n";
    ...
    //
      ...
      std::cout << "inside ELSE HARTLEY'S METHOD\n";
    ...
    for (i = 0; i < nframes; i++) {
      ...
      if (!isVerticalStereo || !useUncalibrated) {
        ...
        // Buindling 3D image
        cv::Mat disparityMap; // Load your disparity map here
        // std::cout << "Q:\n" << Q << std::endl; 
        cv::Mat points3D;
        cv::reprojectImageTo3D(disp, points3D, Q, false, -1);
        cv::imshow("points3D", points3D);
